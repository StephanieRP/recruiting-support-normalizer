...
describe("San Francisco Normalizer", () => {
  describe.skip("Unit Type Normalizer", () => {
    ...
    it("Correctly normalizes command units.", () => {
      expect(incident.normalizeUnitType("CV380")).to.equal(
        "Mobile Command Post"
      );
    });
  ...
  describe("Basic Incident", () => {
    ...
    before(() => {
      b05 = new FireIncident(
        JSON.parse(
          fs.readFileSync(path.join(__dirname, "./data/171743725-B05.json"))
        )
      );
      e05 = new FireIncident(
        JSON.parse(
          fs.readFileSync(path.join(__dirname, "./data/171743725-E05.json"))
        )
      );
      e06 = new FireIncident(
        JSON.parse(
          fs.readFileSync(path.join(__dirname, "./data/171743725-E06.json"))
        )
      );
      t06 = new FireIncident(
        JSON.parse(
          fs.readFileSync(path.join(__dirname, "./data/171743725-T06.json"))
        )
      );
    });
    ...
    describe("correctly parses the address", () => {
      ...
      it("correctly parses the supervisor district", () => {
        expect(address.location.supervisor_district).to.equal("5");
      });
    });
    ...
    describe("correctly parses the description", () => {
      ...
      it("correctly parses the psap answer time", () => {
        expect(description.psap_answer_time).to.equal(
          "2017-06-23T20:30:09-07:00"
        );
      });
    ...
    describe("correctly parses the apparatus", () => {
      ...
      it("correctly parses the unit's dispatch time", () => {
        expect(b05a.unit_status.dispatched.timestamp).to.equal(
          "2017-06-23T20:34:06-07:00"
        );
        expect(e05a.unit_status.dispatched.timestamp).to.equal(
          "2017-06-23T20:37:59-07:00"
        );
        expect(e06a.unit_status.dispatched.timestamp).to.equal(
          "2017-06-23T20:34:06-07:00"
        );
        expect(t06a.unit_status.dispatched.timestamp).to.equal(
          "2017-06-23T20:34:06-07:00"
        );
      });
      ...
      it("correctly parses the unit's enroute time", () => {
        ...
        expect(e05a.unit_status.enroute.timestamp).to.equal(
          "2017-06-23T20:37:59-07:00"
        );
        expect(e06a.unit_status.enroute.timestamp).to.equal(
          "2017-06-23T20:36:08-07:00"
        );
      ...
      it("correctly parses the unit's arrival time", () => {
        ...
        expect(e05a.unit_status.arrived.timestamp).to.equal(
          "2017-06-23T20:41:38-07:00"
        );
      ...
      it("correctly parses the unit's available time", () => {
        expect(b05a.unit_status.available.timestamp).to.equal(
          "2017-06-23T20:35:46-07:00"
        );
        expect(e05a.unit_status.available.timestamp).to.equal(
          "2017-06-23T20:53:27-07:00"
        );
        expect(e06a.unit_status.available.timestamp).to.equal(
          "2017-06-23T20:37:59-07:00"
        );
        expect(t06a.unit_status.available.timestamp).to.equal(
          "2017-06-23T20:35:46-07:00"
        );
      });
  ...
  describe("correctly parses the apparatus", () => {
    it("lookups erf", () => {
      expect(FireIncident.lookupClassAndRisk()).to.deep.equal({
        risk_category: "Unknown",
        response_class: "Unknown"
      });
      expect(
        FireIncident.lookupClassAndRisk("Fire", "Vehicle Fire")
      ).to.deep.equal({ risk_category: "HIGH", response_class: "Fire" });
      expect(
        FireIncident.lookupClassAndRisk(
          "Non Life-threatening",
          "Medical Incident"
        )
      ).to.deep.equal({ risk_category: "LOW", response_class: "EMS" });
      expect(
        FireIncident.lookupClassAndRisk(
          "Potentially Life-Threatening",
          "Medical Incident"
        )
      ).to.deep.equal({ risk_category: "MEDIUM", response_class: "EMS" });
      expect(FireIncident.lookupClassAndRisk("Alarm", "Alarms")).to.deep.equal({
        risk_category: "LOW",
